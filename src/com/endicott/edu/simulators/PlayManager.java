    ...
    public static void handleTimeChange(String collegeId, int hoursAlive, PopupEventManager popupManager) {
        ...
        if (play != null) {// Is play in production
            ...
            if (play.getState() == PlayModel.PlayState.DIRECTOR_PICKED && play.getDaysSincePurchase() == 3 && play.getDirector().equals("Pro")){
                ...
                if (chanceNumber > 20){
                    popupManager.newPopupEvent(collegeId,"Showtime", "The show has just been performed, it was a hit!", "Yay!", "hit_show", "smile", "great show");
                    ...
                    popupManager.newPopupEvent(collegeId,"Oh no!", "Right before the show started, your lead actor broke his leg.  His understudy was forced to step in.  The show did not do so well...", "oof", "broken_leg", "HEALTH", "hospital visit required");
                    play.setPayout(play.getPayout() - 1000);
                    Accountant.receiveIncome(collegeId, "Drama Club Performance", play.getPayout());
                    playDao.deletePlay(collegeId);
                    isDone = true;
            ...
            if(inventoryManager.isPurchased("Mainstage Production", collegeId) && !isDone) {
                play = new PlayModel();
                playDao.saveThePlay(collegeId, play);
                popupManager.newPopupEvent(collegeId,"It Begins!",
